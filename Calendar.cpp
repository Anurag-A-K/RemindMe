...
void clok::display()
  {
    cout<<"The time set for alarm is : "<<hh<<":"<<mm<<":"<<ss;
    cout<<"\n\t\t\tSTANDING BY!";
  }
...
void al()
{
    ...
    struct node *root  = newNode(639);
     root->left=newNode(520);
     root->right=newNode(350);
     root->left->left=newNode(850);
     root->left->right=newNode(780);
	 root->left->left->left=newNode(453);
     printPreorder(root);
     printInorder(root);  
     printPostorder(root);
...
int main(){
    ...
    SetConsoleTitle("DSA Project-17BCE0544,17BCE0547");
    int choice;
    char ch = 'a';
    while(1){
        system("cls");
        printf("\t\t\t\t\tREMAINDER MENU\n\t1. Find Out the Day\n");
